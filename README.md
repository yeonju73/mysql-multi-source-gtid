...
이 프로젝트는 Docker 컨테이너 간의 원활한 이름 기반 통신을 위해 **사용자 정의 브리지 네트워크**를 생성하고 각 프로세스를 연결합니다.

#### [Step 1] Docker 네트워크 생성

기본 `bridge` 네트워크는 컨테이너 이름으로 서로를 찾을 수 없으므로, 별도의 네트워크를 생성합니다.
...
각 컨테이너를 생성한 네트워크에 할당하여 실행합니다.
*참고: `-v` 옵션을 통해 설정 파일(`my.cnf`)을 마운트하거나 직접 환경 변수를 설정할 수 있습니다.*
...
레플리카 컨테이너에서 각 마스터 서버로의 접근 가능 여부를 확인합니다.

```bash
# 레플리카 내부에서 마스터 2로 접속 테스트 (MySQL 클라이언트 도구 사용)
